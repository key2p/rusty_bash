...
impl From<&ArithError> for String {
    fn from(e: &ArithError) -> String {
        match e {
            ArithError::AssignmentToNonVariable(right) => error_msg("attempted assignment to non-variable", right),
            ArithError::DivZero(token) => error_msg("division by 0", token),
            ArithError::Exponent(s) => error_msg("exponent less than 0", &s.to_string()),
            ArithError::NoColon(token) => error_msg("`:' expected for conditional expression", token),
            ...
            ArithError::OperandExpected(token) => error_msg("syntax error: operand expected", token),
