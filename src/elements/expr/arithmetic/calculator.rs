...
use super::{
    elem::{ArithElem, float, int, ternary, variable},
    rev_polish,
};
use crate::{
    ShellCore,
...
pub fn pop_operands(stack: &mut Vec<ArithElem>, core: &mut ShellCore) -> Result<(ArithElem, ArithElem), ExecError> {
    let right = stack.pop();
    let left = stack.pop();
...
fn bin_calc_and_or(op: &str, stack: &mut Vec<ArithElem>, core: &mut ShellCore) -> Result<(), ExecError> {
...
fn bin_calc_operation(op: &str, stack: &mut Vec<ArithElem>, core: &mut ShellCore) -> Result<(), ExecError> {
