...
use super::{
    ArithElem, ArithmeticExpr,
    elem::{float, int},
};
use crate::{
    Feeder, ShellCore,
    elements::{
        substitution::subscript::Subscript,
        word::{Word, WordMode},
    },
...
impl ArithmeticExpr {
    ...
    fn eat_num(feeder: &mut Feeder, ans: &mut Self, core: &mut ShellCore) -> Result<bool, ExecError> {
    ...
    fn eat_conditional_op(feeder: &mut Feeder, ans: &mut Self, core: &mut ShellCore) -> Result<bool, ExecError> {
        if !feeder.starts_with("?") {
            return Ok(false);
        }
    ...
    fn eat_paren_internal(feeder: &mut Feeder, core: &mut ShellCore, ans: &mut Self) -> Result<bool, ExecError> {
        if !feeder.starts_with("(") {
            return Ok(false);
        }
    ...
    pub fn parse_after_eval(feeder: &mut Feeder, core: &mut ShellCore, left: &str) -> Result<Option<Self>, ExecError> {
