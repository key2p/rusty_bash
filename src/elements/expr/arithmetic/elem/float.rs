...
pub fn bin_calc(op: &str, left: f64, right: f64, stack: &mut Vec<ArithElem>) -> Result<(), ExecError> {
    let bool_to_01 = |b| {
        if b { ArithElem::Integer(1) } else { ArithElem::Integer(0) }
    };
