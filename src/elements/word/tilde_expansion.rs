...
use super::WordMode;
use crate::{ShellCore, elements::word::Word, error::exec::ExecError};
...
fn eval_single(word: &mut Word, core: &mut ShellCore) {
    ...
    let text: String =
        word.subwords[1..length].iter().map(|e| e.get_text().to_string()).collect::<Vec<String>>().concat();
...
pub fn eval_multi(word: &mut Word, core: &mut ShellCore) {
    ...
    word.text = word.subwords.iter().map(|e| e.get_text().to_string()).collect::<Vec<String>>().concat();
}
