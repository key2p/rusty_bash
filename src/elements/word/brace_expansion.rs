...
use crate::elements::{
    subword::{Subword, single_quoted::SingleQuoted},
    word::Word,
};
...
fn ascii_to_subword(c: char) -> Box<dyn Subword> {
    let table = vec![
        "^?", "\\M-^@", "\\M-^A", "\\M-^B", "\\M-^C", "\\M-^D", "\\M-^E", "\\M-^F", "\\M-^G", "\\M-^H", "\\M-\t",
        "\\M-\n", "\\M-^K", "\\M-^L", "\\M-^M", "\\M-^N", "\\M-^O", "\\M-^P", "\\M-^Q", "\\M-^R", "\\M-^S", "\\M-^T",
        "\\M-^U", "\\M-^V", "\\M-^W", "\\M-^X", "\\M-^Y", "\\M-^Z", "\\M-^[", "\\M-^\\", "\\M-^]", "\\M-^^", "\\M-^_",
        " ", "¡",
    ];

    let n = c as usize;
    let text = if n >= 127 && n < 127 + table.len() { table[n - 127].to_string() } else { c.to_string() };
...
fn get_delimiters(stack: &mut Vec<Option<&str>>) -> Option<(Vec<usize>, BraceType)> {
    ...
    if period_pos.len() == 4 && period_pos[0] == period_pos[1] + 1 && period_pos[2] == period_pos[3] + 1 {
        period_pos.reverse();
        period_pos.insert(0, 0);
        period_pos.push(stack.len() - 1);
...
fn comma_brace_to_subwords(subwords: &Vec<Box<dyn Subword>>, delimiters: &Vec<usize>) -> Vec<Vec<Box<dyn Subword>>> {
    let mut ans = vec![];
...
fn expand_comma_brace(subwords: &Vec<Box<dyn Subword>>, delimiters: &Vec<usize>, compat_bash: bool) -> Vec<Word> {
