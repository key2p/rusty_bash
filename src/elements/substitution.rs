...
use self::{value::Value, variable::Variable};
use crate::{
    Feeder, ShellCore,
...
impl Substitution {
    ...
    fn set_array_elem(&mut self, core: &mut ShellCore, layer: usize, index: &String) -> Result<(), ExecError> {
        if index.is_empty() {
    ...
    fn eat_left_hand(&mut self, feeder: &mut Feeder, core: &mut ShellCore) -> Result<bool, ParseError> {
    ...
    pub fn parse_as_arg(feeder: &mut Feeder, core: &mut ShellCore) -> Result<Option<Self>, ParseError> {
        // if feeder.starts_with("\"")
        // && ! feeder.starts_with("\"-") {
    ...
    pub fn parse(feeder: &mut Feeder, core: &mut ShellCore, permit_space: bool) -> Result<Option<Self>, ParseError> {
        let mut ans = Self::default();
