...
use crate::{
    Feeder, ShellCore,
    elements::{
        subword::{SingleQuoted, Subword},
        word::{Word, WordMode},
    },
    error::{arith::ArithError, exec::ExecError, parse::ParseError},
    utils,
};
...
impl OptionalOperation for ValueCheck {
    ...
    fn exec(&mut self, variable: &Variable, text: &String, core: &mut ShellCore) -> Result<String, ExecError> {
    ...
    fn set_heredoc_flag(&mut self) {
        self.alternative_value.iter_mut().for_each(|e| e.set_heredoc_flag());
    }
...
impl ValueCheck {
    ...
    fn set_value(&mut self, variable: &Variable, core: &mut ShellCore) -> Result<String, ExecError> {
