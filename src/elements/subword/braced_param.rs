...
use crate::{
    Feeder, ShellCore,
    elements::{substitution::variable::Variable, subword::Subword},
...
impl Subword for BracedParam {
        ...
        if self.param.is_array() {
            if let Some(s) = self.optional_operation.as_mut() {
                if s.has_array_replace() {
                    ...
                    self.array = Some(arr);
                    if self.param.index.is_some() && self.param.index.as_ref().unwrap().text == "[*]" {
    ...
    fn split(&self, ifs: &str, prev_char: Option<char>) -> Vec<(Box<dyn Subword>, bool)> {
        ...
        let index_is_asterisk = self.param.index.is_some() && self.param.index.as_ref().unwrap().text == "[*]";
        ...
        if (!self.treat_as_array && !index_is_asterisk && self.param.name != "*")
            ...
            || self.array.is_none()
        {
            return splitter::split(&self.text, ifs, prev_char).iter().map(|s| (From::from(&s.0), s.1)).collect();
        }
    ...
    fn set_heredoc_flag(&mut self) {
        self.optional_operation.iter_mut().for_each(|e| e.set_heredoc_flag());
    }
}
