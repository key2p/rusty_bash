...
use self::{
    ansi_c_quoted::AnsiCQuoted, arithmetic::Arithmetic, braced_param::BracedParam, command_sub::CommandSubstitution,
    double_quoted::DoubleQuoted, ext_glob::ExtGlob, file_input::FileInput, filler::FillerSubword, parameter::Parameter,
    paren::EvalLetParen, process_sub::ProcessSubstitution, simple::SimpleSubword, single_quoted::SingleQuoted,
    varname::VarName,
};
use crate::{
    Feeder, ShellCore,
    elements::word::WordMode,
...
pub trait Subword {
    ...
    fn split(&self, ifs: &str, prev_char: Option<char>) -> Vec<(Box<dyn Subword>, bool)> {
        ...
        splitter::split(&self.get_text(), ifs, prev_char).iter().map(|s| (From::from(&s.0), s.1)).collect()
    }
...
pub fn parse_special_subword(
    ...
    match mode {
        ...
        Some(WordMode::ReparseOfValue) => {
            ...
            if feeder.len() == 0 { Ok(None) } else { Ok(Some(From::from(&feeder.consume(1)))) }
        },
        _ => Ok(None),
    }
}
