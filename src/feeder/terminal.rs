...
use crate::{ShellCore, error::input::InputError, file_check, utils::file};
...
impl Terminal {
    ...
    fn make_prompt_string(raw: &str) -> String {
        ...
        raw.replace("\\u", &user).replace("\\h", &hostname).replace("\\w", &cwd).replace("\\b", &branch).to_string()
    }
    ...
    pub fn shift_cursor(&mut self, shift: i32) {
        let prev = self.head;
        Self::shift_in_range(&mut self.head, shift, self.prompt.chars().count(), self.chars.len());
    ...
    fn completion_finish_check(&mut self) {
        match self.prev_key {
            event::Key::Char('\t') | event::Key::Left | event::Key::Down | event::Key::Right | event::Key::Up => return,
            _ => {
                self.tab_num = 0;
