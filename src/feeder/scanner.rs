...
impl Feeder {
    ...
    fn scanner_chars(&mut self, judge: fn(char) -> bool, core: &mut ShellCore, skip_bytes: usize) -> usize {
        loop {
            let mut ans = 0;
    ...
    pub fn scanner_ansi_c_hex(&mut self, core: &mut ShellCore) -> usize {
        ...
        let judge = |ch| ('0' <= ch && ch <= '9') || ('a' <= ch && ch <= 'f') || ('A' <= ch && ch <= 'F');
    ...
    pub fn scanner_arith_number(&mut self, core: &mut ShellCore) -> usize {
        let judge = |ch| {
            ('0' <= ch && ch <= '9') || ('a' <= ch && ch <= 'z') || ('A' <= ch && ch <= 'Z') || ".#xX_@".contains(ch)
        };
        self.scanner_chars(judge, core, 0)
    }
    ...
    pub fn scanner_name(&mut self, core: &mut ShellCore) -> usize {
        ...
        let judge = |ch| ch == '_' || ('0' <= ch && ch <= '9') || ('a' <= ch && ch <= 'z') || ('A' <= ch && ch <= 'Z');
        self.scanner_chars(judge, core, 0)
    }
